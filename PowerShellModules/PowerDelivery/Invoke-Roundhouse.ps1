...
function Invoke-Roundhouse {
    ...
    if ([String]::IsNullOrWhiteSpace($connectionString)) {
        Write-BuildSummaryMessage "$scriptsDir -> $database ($server)"
    }
    else {
        Write-BuildSummaryMessage "$scriptsDir -> `"$connectionString`""
    }
}

Export-ModuleMember -Function Invoke-Roundhouse