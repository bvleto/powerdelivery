...
function Publish-SSAS {
  ...
  foreach ($curComputerName in $computerNames) {
    ...
    if (![String]::IsNullOrWhiteSpace($cubeName)) {
      ...
      Push-BuildAssets -computerName $curComputerName -path $asFilesDir -destination $asFilesDir -DriveLetter $driveLetter
      ...
      $outRemoteDeployPath = Join-Path $remoteDeployPath ($xmlaPath -replace ".xmla", "ExecutionLog.xml")
        ...
        if ($deploySuccess) {
            Write-BuildSummaryMessage "$($asModelName).asdatabase -> $newModelName ($curComputerName)"
        }
        else {
...
Export-ModuleMember -Function Publish-SSAS